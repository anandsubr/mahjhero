/*
 * A club thread's title is just the club's own name now, not
 * `'Everyone at ' || c.name`.
 *
 * The `1C thread` artboard renders `'Everyone at ' + club.short` from a
 * SHORT club name its fixture carries (`short: 'Riverside'`) — a column
 * this app's `clubs` table has never had. Substituting the full name
 * produced "Everyone at Riverside Mah Jongg", which wraps to two lines in
 * the thread screen's own header and truncates in the messages list. The
 * owner's call, made knowingly: drop the "Everyone at " prefix and use the
 * club's name alone.
 *
 * `create or replace function` rather than a rename-and-recreate: the
 * return table's column names and types are unchanged, only the `title`
 * expression's one CASE branch differs, and replacing in place keeps the
 * function's OID (and therefore its existing grants) stable — see the
 * original migration's closing `grant execute ... to authenticated`, which
 * this does not need to repeat.
 *
 * lib/messages.ts's `threadTitleFor` makes the identical change for the
 * thread screen's own header, in the same commit — see that function's own
 * comment for why both places have to move together, or the list and the
 * header would say two different things about the same thread.
 */
