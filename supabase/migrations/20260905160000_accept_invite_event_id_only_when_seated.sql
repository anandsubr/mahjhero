/*
 * Whole-branch review finding: accept_club_invite's final jsonb_build_object
 * returned `invite.event_id` unconditionally, regardless of whether the
 * nested seating attempt above it actually succeeded. When seating is
 * skipped (the event was since cancelled/ended, or any other reason the
 * `exception when others then null;` block swallows), the guest still got
 * redirected to an event they now have no booking on and cannot see --
 * app/join/[token].tsx sends them straight to
 * /clubs/[clubId]/events/[eventId], which renders "That game could not be
 * loaded" with no way out.
 *
 * Fix: only include event_id in the response when a booking group was
 * actually created for this guest. No client-side change needed --
 * app/join/[token].tsx already falls back to the plain club redirect
 * (`/clubs/${clubId}`) when `eventId` is null/falsy.
 *
 * `create or replace`, not drop+create: the signature and return type
 * (jsonb) are unchanged, only the body's final return-value logic changes.
 */
