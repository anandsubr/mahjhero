/*
 * Finding #6 of the final whole-branch review: create_event/create_event_series
 * (20260906120000_seating_mode_create.sql) refuse a capacity under one --
 * `raise exception 'capacity must be at least one' using errcode = '23514'`
 * -- which lib/events.ts's error-mapping table already turns into "Capacity
 * must be at least one player." for the host. update_event/
 * update_event_series (20260906130000_seating_mode_propagation.sql) never
 * grew the same guard, so the identical bad input (0, or a negative number)
 * takes a different path on each screen: refused with a friendly message on
 * create, but on edit it falls straight through to the `events`/
 * `event_series` table's own `capacity` check constraint -- also `23514`,
 * but with a raw constraint-violation message the client's mapping table
 * does not recognize, so it surfaces as the generic error. Same input,
 * different experience depending on which screen the host happened to be
 * on.
 *
 * Both signatures are unchanged, so both are `create or replace`, not
 * drop-and-recreate. Bodies are copied verbatim from
 * 20260906130000_seating_mode_propagation.sql, with exactly one new
 * three-line guard added to each, placed right after `eff_capacity` is
 * resolved (mirroring where create_event's own guard sits, right after its
 * `capacity_limit` is in scope) and before any other validation. The guard
 * reads `eff_capacity`, not the raw `new_capacity`/`clear_capacity`
 * arguments, for the same reason the rest of each function already does:
 * `eff_capacity` is what actually resolves clear_capacity vs. "leave
 * alone", so a clear (`clear_capacity => true`, `eff_capacity` ends up
 * `null`) is correctly exempt -- clearing to uncapped is not "a capacity
 * under one", it is no capacity at all.
 */
...
begin
  ...
  -- The new guard (this migration). Mirrors create_event's own
  -- `capacity_limit is not null and capacity_limit < 1` check
  -- (20260906120000_seating_mode_create.sql), reading the resolved
  -- `eff_capacity` rather than the raw argument for the reason the
  -- file-level comment above gives.
