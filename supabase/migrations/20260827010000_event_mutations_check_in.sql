 ...
 *
 * Bodies copied byte-for-byte from each function's LAST definition, not the
 * migration that first introduced it — grepping the migrations directory
 * for each function's definition shows create_event, update_event and
 * create_event_series were all last redefined by
 * 20260824001000_reject_invisible_past_dated_games.sql (adding the
 * past-start guard), and update_event_series by
 * 20260825042000_series_shortening_tells_the_booked.sql (adding the
 * shortening notification). Copying from the migrations named in the task
 * brief (20260823070000 / 20260823030000) would have silently dropped both
 * of those fixes, the same trap Task 1's own migration comment describes
 * hitting from the materialize_one_series side.
 */
...
drop function public.update_event(
  uuid, text, uuid, text, date, time, int);
