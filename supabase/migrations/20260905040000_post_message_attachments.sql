 ...
 *
 * The body below is 20260830011000_quote_stays_in_post.sql's current text
 * (the most recent migration to touch post_message -- NOT
 * 20260830010000_post_message_roots.sql, which quote_stays_in_post already
 * superseded) with only two kinds of change layered on: the empty-body
 * guard now also accepts a nonempty attachment array, and an attachment
 * loop runs after the message row is inserted. The quote-scoping logic,
 * guard ordering, and the two post-insert updates (last_message_at, then
 * the reply-count bump) are otherwise untouched.
 */

/*
 * messages.body's own check constraint (20260829000000) still enforces a
 * length of at least 1 -- the RPC-level guard above cannot satisfy "empty
 * body with an attachment" on its own if the table itself refuses the row.
 * Only the lower bound moves; 2000 stays the upper bound both here and in
 * the RPC's own belt-and-braces length check.
 */
alter table public.messages
  drop constraint messages_body_check;
alter table public.messages
  add constraint messages_body_check check (length(trim(body)) between 0 and 2000);
