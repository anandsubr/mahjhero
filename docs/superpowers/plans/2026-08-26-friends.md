...
export default function FriendsScreen() {
  ...
  // native double-tap. This ref is written synchronously, so it is what
  // actually makes the guard sound; `busy` keeps driving the disabled look
  // of the controls. app/clubs/index.tsx carries the same pair for the same
  // reason.
  ...
  // Keyed on the user id, NOT on `session`: lib/session.tsx hands out a
  // fresh Session object on every onAuthStateChange, TOKEN_REFRESHED
  // included, which fires within the hour and on web tab focus. Depending on
  // the object refetches every time for a value that only changes on a real
  // account switch — and two in-flight loads clobber each other's state.
  // app/profile.tsx and lib/use-viewer.ts both record this same trap.
