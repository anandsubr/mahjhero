...
- **A held offer keeps a seat empty for up to two hours** while a club may be a player short. That is the parent spec's deliberate trade. The expiry caps how long any ONE offer holds its seats, not the group's place in line: a lapsed, unanswered offer is treated like "does not fit" — the group keeps its `waitlisted_at` and is not re-offered that same partial fit, but stays eligible to be seated outright the moment enough seats are free at once. (`promote_waitlist` originally re-offered the same seats the instant an offer lapsed, which could starve everyone waiting behind that group indefinitely — fixed at the whole-branch review, 20260825090000.)
