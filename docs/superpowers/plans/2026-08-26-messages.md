...
- **A fixture that looks a row up while running as `authenticated` gets RLS applied to the lookup.** A `select id from … where title = …` as a non-member returns NULL, and the assertion then fires the wrong error code and passes for the wrong reason. Capture the id into a temp table while still running as a member, or look it up before switching identity. Found in messages Task 4, where it made a `42501` assertion pass on a `22023`.
