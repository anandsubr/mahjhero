...
**Interfaces:**
- Consumes: `lib/messages`'s `fetchThread`, `fetchThreadMessages`, `postMessage`, `markThreadRead`, `deriveSubject`, `quoteStub`, `threadTitleFor`, `leaveGroupThread`; `lib/supabase`'s `supabase.channel`.
...
**Keep the `broadcasts` table and the type.** The Edge Function's renderer reads the row, `broadcast_id` on `messages` points at it, and it anchors the per-recipient dedupe key. Keep `countBroadcastRecipients` too: nothing calls it in this plan, but it is the count a future "this goes to N members" confirmation needs, and it is three lines. If the linter objects to an unused export, that is a signal to wire the confirmation now rather than to delete the function.
...
Run: `npm test`
Expected: PASS. Every screen that renders `TabBar` now calls `fetchUnreadCounts`, so any test mocking `lib/messages` partially will need `fetchUnreadCounts` added to its mock. That is the cost of the badge and it is one line per file.
