...
- **Run `npm test` as well as `npm run test:db` on every database task.** `lib/bookings.test.ts` runs a self-audit that greps every migration for `raise exception '…'` and fails unless each message is mapped in `BOOKING_REFUSALS` or justified in that test's own `ALLOWLIST`. A migration that adds a refusal turns the vitest suite red while the pgTAP suite stays green. This went unnoticed for two tasks.
- **A task that changes a screen must regenerate that screen's visual baselines in the same commit.** `e2e/visual.spec.ts-snapshots/` holds a PNG per screen per viewport. Changing a screen without refreshing them leaves the visual suite red for every later task, which trains people to ignore it. Always inspect the diff image before regenerating — a baseline refreshed blindly bakes in whatever regressed. Friends Task 6 changed the profile screen and did not, and the suite was red for two tasks.
- **A function named in an RLS policy must be granted to `authenticated`.** A policy's `USING` expression is evaluated as the querying user, not as the table owner, so an ungranted function makes an ordinary select fail with `permission denied for function`. `is_booking_group_member` (20260825000000) is the precedent. Functions called only from `security definer` RPC bodies stay revoked — those run as owner. Discovered during messages Task 2.
- **Never nest `do $x$ … $x$` inside a `$$…$$` throws_ok/lives_ok argument.** Postgres matches dollar-quote delimiters by substring, so `$x$$$` closes the OUTER `$$` two characters early and leaves a stray `$` — a real syntax error. Use a distinct outer tag (`$test$…$test$`). Found in messages Task 4.
- **A fixture that looks a row up while running as `authenticated` gets RLS applied to the lookup.** A `select id from … where title = …` as a non-member returns NULL, so the RPC receives NULL and raises a different refusal than the one the assertion is about. Capture the id into a temp table while still running as a member, or look it up before switching identity. Found in messages Task 4, where a `42501` assertion caught `22023` instead — it failed loudly rather than passing wrongly, but the assertion was testing the wrong branch either way.
- **`now()` is constant for a whole transaction, and a pgTAP fixture is one transaction.** Seed a membership and post a message in the same fixture and their timestamps TIE — so an unread predicate written `created_at > greatest(joined_at, …)` counts nothing, because nothing is strictly greater. Set `club_members.joined_at` explicitly to a past instant when the fixture needs a message to be newer than a join. `waitlist_promotion.test.sql` already does this. Found in messages Task 6. This is a fixture artefact only: in production a join and a message are different transactions.
- **Mock `expo-router` and `lib/session` HONESTLY, or the screen grows defensive code to survive the mock.** Two traps, both hit in messages Task 12: (1) `useFocusEffect: (cb) => cb()` fires on every render, which the real hook never does — wrap it in a `useEffect` keyed on the callback identity. (2) A `useSession` mock returning a fresh object literal each render breaks referential stability the real Context provides, and produces a genuine render loop once the focus mock is honest — hoist the session to a module-scoped constant. A guard added to production code to survive a bad double is a defect, not a fix.
 ...
 *
 * This runs BEFORE the event gets its own successful game thread below, and
 * the order is load-bearing: `message_threads_one_per_event` is a unique
 * index on event_id alone, so once that insert claims the event, ANY second
 * row naming it raises 23505 before the composite foreign key is ever
 * reached — masking the very violation this assertion exists to prove.
 */
select throws_ok(
...
-- nothing by default.
--
-- BUT can_read_thread is GRANTED to authenticated, and that is not a lapse.
-- An RLS policy's USING expression is evaluated AS THE QUERYING USER, not
-- as the table owner, so a caller with no execute privilege on a function
-- named in a policy gets `permission denied for function` on an ordinary
-- select. 20260825000000_create_bookings.sql sets the precedent exactly:
-- is_booking_group_member is revoked from public and anon and granted to
-- authenticated for this same reason.
--
-- Granting it leaks nothing: it answers "may I read this thread" about the
-- CALLER, which the caller can already discover by selecting. The other two
-- are called only from RPC bodies — security definer, running as owner — so
-- they stay revoked.
-- ---------------------------------------------------------------------
...
-- Riverside's active members by this point in the accumulated fixture are
-- Alice, Bob and Dave; Alice is the author and is excluded. Recount if the
-- seeds above ever change — this number is a fact about the fixture, not
-- about the function.
select is(
...
Note the `recipient_count` of 2: Riverside's active members at this point are Alice, Bob and Dave, and the author Alice is excluded. This is a fact about the accumulated fixture, not about the function — if the seeds change, recount rather than changing `post_message`.
...
In `thread_lists.test.sql`, change `select plan(9);` to `select plan(11);` and add the block below.

**Placement matters, and the obvious spot is wrong.** Putting it before the first `set local role authenticated;` makes it run before Task 6's own assertions, and the threads it creates break two of them — "an empty club thread is listed before anybody has posted" and "an opened but unused game thread stays out of the list" both start seeing an extra row. Put it AFTER those two assertions instead, bracketed by `reset role;` … `set local role authenticated;` so the backfill function (revoked from `authenticated`) is still called as the owner:
...
**Seed TWO broadcasts, not one:** the event-targeted one above and a club-wide one with `event_id` null, each with its own assertion. With only the event-targeted case covered, that line could regress to `=` and lose every club announcement in the app's history with the suite still green — and this backfill is the entire reason deleting the broadcast screens in Task 15 is defensible. Prove the new assertion is not vacuous by temporarily changing the join to `=` and confirming only it fails.
...
-- GRANTED, not revoked, and deliberately: RLS evaluates a policy's USING
-- expression as the querying user, so a caller without execute on this gets
-- `permission denied for function` on an ordinary select. Same posture, and
-- the same reason, as is_booking_group_member (20260825000000).
select ok(
  has_function_privilege(
    'authenticated', 'public.can_read_thread(uuid)', 'EXECUTE'),
  'can_read_thread is executable by authenticated, because a policy calls it'
);
select ok(
...
### Task 10b: `fetch_thread_messages` and `thread_roster` *(inserted during execution)*

**Why this task exists.** It is not in the original plan. Task 10 discovered, against a real database, that `profiles` RLS is **self-row-only** — `20260822180000_club_roster_narrow_profiles.sql` deliberately dropped the co-member policy and introduced the `club_roster()` definer RPC in its place. So a PostgREST embed of `profiles(display_name)` resolves to NULL for anybody but the caller, and the thread screen cannot render sender names or a group roster. The same task also proved PostgREST cannot embed `messages`' composite self-reference at all, forcing a second query for quoted parents.

One migration fixes both, using the pattern `fetch_my_threads` already uses: join `profiles` inside a `security definer` body, where RLS does not apply.

**Files:**
- Create: `supabase/migrations/20260829080000_thread_reads_api.sql`
- Create: `supabase/tests/database/fixtures/thread_reads_api.test.sql`
- Modify: `lib/messages.ts` — `fetchThreadMessages` and `fetchThread`'s roster now go through the RPCs
- Modify: `lib/messages.test.ts`, `lib/schema-contract.test.ts` — the pinned "profiles is null" assertions must now assert the fixed behaviour
- Modify: `supabase/tests/database/portable/grants.test.sql` — two new signatures in both allowlist arrays

**Produces:**
- `fetch_thread_messages(target_thread uuid) returns table (id uuid, author_id uuid, author_name text, body text, subject text, is_announcement boolean, created_at timestamptz, reply_to_id uuid, reply_to_body text, reply_to_author text)` — gated by `can_read_thread(target_thread)`, raising `42501` otherwise. Resolves the quoted parent in the same query, which also removes Task 10's second round trip.
- `thread_roster(target_thread uuid) returns table (profile_id uuid, display_name text)` — gated the same way. Returns the group's members; empty for a club or game thread, whose membership is derived rather than materialised.

Both `security definer`, both granted to `authenticated`, both added to the closed-world allowlist arrays.

**Do NOT widen `profiles` RLS.** `20260822180000` narrowed it deliberately, to close a leak its own comment describes. Reopening it would undo that.

The pgTAP fixture must prove the thing the RLS gap made impossible: **a member reads another member's display name** through these RPCs, in a group thread whose members do not share a club — the cross-club case the friends feature exists to make possible. And it must prove a non-member gets `42501` rather than an empty result, so a thread id cannot be probed.
 ...
 *
 * `accent[700]`, NOT the artboard's `accentColor`: cream on accentColor
 * measures 3.03:1, and this text is 16px bold — under the 14pt-bold "large
 * text" threshold, so it needs 4.5:1 and fails outright. accent[700] reads
 * 5.72:1. Same pairing, same failure and same fix as components/TabBar.tsx's
 * selected tab. lib/theme.test.ts pins the ratio.
 */
 ...
 *   direct  a person glyph
 *   group   a person glyph too
 */
...
export default function ThreadScreen() {
  ...
  // Written synchronously, because `sending` above is read from the render
  // closure and is blind to a second activation in the same tick. For an
  // ordinary message that is a duplicate line; for an announcement it is a
  // second email to the whole club, which cannot be unsent. Same guard, and
  // the same reasoning, as app/clubs/index.tsx and app/friends.tsx.
              ...
              return (
                ...
                >
                  ...
                  */}
                  ...
                  {/*
                    The ANNOUNCEMENT branch wins over `mine`, because its
                    background does. Keying the text off `mine` alone put
                    cream on pale green at 1.10:1 — invisible, and it happened
                    to every organizer looking at their own announcement.
                  */}
                  <Text
                    style={
                      m.is_announcement
...
const styles = StyleSheet.create({
  ...
  // accent[700], not accentColor: cream on accentColor is 3.03:1 and this
  // body text is 18px regular, needing 4.5:1. accent[700] reads 5.72:1 —
  // the same failure and the same fix as UnreadBadge and TabBar.
  ...
  send: {
    ...
    borderRadius: radius.pill,
    // See `mine` above — same pairing, same measured failure.
    backgroundColor: colors.accent[700],
    alignItems: 'center',
    justifyContent: 'center',
  },
...
export default function NewMessageScreen() {
  ...
  // Written synchronously: `busy` is read from the render closure and is
  // blind to a second activation in the same tick, which here would create
  // two group threads. Fifth instance of this guard in the codebase — see
  // app/clubs/index.tsx, app/friends.tsx and both other message screens.
  ...
  const start = useCallback(async () => {
    ...
    // The empty-picked case must be caught HERE, not left to
    // createGroupThread: with People selected and nobody ticked, the call
    // would go out and come back refused, which is a round trip to learn
    // something the screen already knows. The copy matches the library's own
    ...
    const result =
      target === 'everyone'
        ...
        : picked.length === 0
          ? { id: null, error: 'Pick somebody to message.' }
        ...
        <>
          {clubs.length > 1 ? (
            <>
              ...
              {/*
                ClubChips' real props are `{ chips, selected, onSelect }` with
                `selected: string` — read components/ClubChips.tsx. The chip
                array is built here rather than with lib/dashboard's
                buildChips, which prepends an "All clubs" pseudo-chip: you
                must pick exactly ONE club to announce to.
              */}
              <ClubChips
...
- [ ] **Step 7: Regenerate the baselines for the two screens you rewired**

`app/clubs/[id]/index.tsx` loses a button (two collapse to one) and `app/clubs/[id]/events/[eventId]/index.tsx` may change its label. Both have stored baselines at both viewports. Run `npm run test:visual`, inspect each diff image, regenerate only what genuinely moved, and confirm the suite is green — including that the four deleted `broadcast-*` baselines leave no orphan behind.
...
export function useUnreadCounts(): UnreadCounts {
  ...
  // a fresh Session on every onAuthStateChange, TOKEN_REFRESHED included,
  // which fires within the hour and on web tab focus. Keying on the object
  // refetches for a value that only changes on a real account switch.
  // lib/use-viewer.ts and app/profile.tsx both record this same trap.
...
- [ ] **Step 6: Regenerate every baseline that contains a tab bar**

This badge changes the pixels of **six** screens, not one. `TabBar` is rendered by `app/profile.tsx`, `app/notifications.tsx`, `app/messages/index.tsx`, `app/clubs/index.tsx`, `app/clubs/[id]/index.tsx` and `app/clubs/[id]/venues.tsx`, and the club chips change `app/clubs/index.tsx` again. Every stored baseline for those screens, at both viewports, is now stale.

Run `npm run test:visual` and let it tell you which baselines moved. **Inspect each diff image before regenerating** — you are looking for the badge and nothing else. A colour shift, a reflow, or a missing control in one of those diffs is a real regression this change caused, and regenerating over it makes it permanent and invisible.

Then `npm run test:visual -- --update-snapshots -g "<the failing tests>"`, look at the new PNGs, and re-run the whole suite to confirm it is green.

Note the badge only renders when something is unread. If the seeded Playwright user has no unread messages, these baselines will be pixel-identical and nothing will move — that is a valid outcome, not a sign the badge is broken. Say which happened in your report.

- [ ] **Step 7: Commit**

```bash
git add lib/use-unread.ts components app/clubs/index.tsx app/__tests__ components/__tests__ e2e
...
- [ ] **Step 2b: Seed an unread message, so the badge is actually pictured**

Task 16 shipped the tab badge and the club-chip badge, and **not one screenshot contains either of them** — the seeded Playwright user has no unread messages, so `UnreadBadge` returns null everywhere and all 32 baselines came back byte-identical. Its real rendering — whether the pill clips the icon, overflows the tab, or collides with the label — is unverified by anything.

In the seeded `describe` block, post a message into a club thread **as a different member** than the one the browser is signed in as (a message you sent yourself is never unread), then capture the dashboard. Use the service-role client `e2e/session.ts` already provides for setup, the same way the other seeds work. Assert the badge is visible before capturing, so a future regression that stops rendering it fails loudly rather than quietly matching an empty baseline.
