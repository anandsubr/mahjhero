...
| `app/messages/[threadId].tsx` | One thread, with Realtime |
...
- Produces, granted to `authenticated`:
  - `post_message(target_thread uuid, p_body text, p_announce boolean default false) returns uuid`
  - `mark_thread_read(target_thread uuid) returns void`

- [ ] **Step 1: Write the failing test**

In `thread_mutations.test.sql`, change `select plan(14);` to `select plan(22);` and add before `finish()`:
...
revoke execute on function public.post_message(uuid, text, boolean) from public, anon;
...
select ok(
  has_function_privilege(
    'authenticated', 'public.post_message(uuid, text, boolean)', 'EXECUTE'),
...
- Produces:
  ...
  - pure: `deriveSubject`, `kindLabel`, `messagePreview`, `unreadLabel`, `sortThreads`, `sectionThreads`, `threadTitleFor`
...
export const MESSAGE_COLUMNS =
  'id, author_id, body, subject, is_announcement, created_at, profiles(display_name)';
...
**Interfaces:**
- Consumes: `lib/messages`'s `fetchThread`, `fetchThreadMessages`, `postMessage`, `markThreadRead`, `deriveSubject`, `threadTitleFor`, `kindLabel`, `leaveGroupThread`; `lib/clubs`'s `canInvite` for the organizer gate; `lib/supabase`'s `supabase.channel`.
...
describe('thread screen', () => {
  ...
  it('sends a message and clears the composer', async () => {
    ...
    await waitFor(() =>
      expect(postMessage).toHaveBeenCalledWith('t1', 'On my way', false),
    );
...
export default function ThreadScreen() {
  ...
  const send = useCallback(async () => {
    ...
    setSending(true);
    setError(null);
    const { error: refusal } = await postMessage(threadId, draft, announce);
    setSending(false);
    if (refusal) {
      // The draft is NOT cleared. Losing what somebody typed because the
      // network failed is the worst possible response to a failed send.
      setError(refusal);
      return;
    }
    setDraft('');
    setAnnounce(false);
    await load();
  }, [sending, threadId, draft, announce, load]);
