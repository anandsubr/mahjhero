...
| Supabase | `mahjhero-dev` (`rzutuhabxzcateutaojo`) | `mahjhero-prod` |
...
3. **If the PR adds a migration:** `npx supabase db push --project-ref <prod-ref>`
...
If a PR changes Edge Functions: `npx supabase functions deploy --project-ref <prod-ref>`
after merging.
...
npx supabase config diff --project-ref <prod-ref>   # read-only; review it
npx supabase config push --project-ref <prod-ref>   # prompts per change
```

Never `config push` to dev: the dev dashboard has drifted from the base
config (see the comment above `[remotes.prod]`).

`config diff` does not compare email templates, so check
...
1. **Create the project** in the Supabase dashboard (same org and region as
   dev). Put its ref into `project_id` under `[remotes.prod]` and commit it.
...
3. **Schema, functions, secrets:**
   ...
   npx supabase secrets set --env-file supabase/.env.prod --project-ref <prod-ref>
   ```
...
5. **Notification drain** — in the prod SQL editor (the cron job is a no-op
   ...
   insert into public.app_config (key, value) values
     ('functions_url', 'https://<prod-ref>.supabase.co/functions/v1'),
     ('drain_secret',  '<DRAIN_SECRET from supabase/.env.prod>');
   ```
6. **Providers outside Supabase** — add
   `https://<prod-ref>.supabase.co/auth/v1/callback` as an authorized
